...
// Tính thời gian bắt đầu kỳ thi theo múi giờ
$exam_start_time = new DateTime($result['start_time']);
$exam_start_time->setTimezone($timezone); // Đặt múi giờ

$exam_duration = $result['duration']; // Duration in minutes
$exam_end_time = clone $exam_start_time; // Sao chép thời gian bắt đầu
$exam_end_time->add(new DateInterval('PT' . $exam_duration . 'M')); // Thêm duration vào thời gian bắt đầu

// Kiểm tra thời gian hiện tại so với thời gian kết thúc kỳ thi
$current_time = new DateTime();
$current_time->setTimezone($timezone); // Đặt múi giờ

// So sánh thời gian hiện tại với thời gian kết thúc kỳ thi (so sánh timestamp)
$show_details = $current_time->getTimestamp() >= $exam_end_time->getTimestamp(); // So sánh thời gian đầy đủ
?>
